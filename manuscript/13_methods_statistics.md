...
{{SWEEP_HEADLINE_METHOD}} has rank-biserial effect
{{SWEEP_BEST_EFFECT_SIZE}}, d-equivalent {{SWEEP_BEST_D_EQUIVALENT}}
({{SWEEP_BEST_EFFECT_LABEL}}), with the paired mean difference and bootstrap
...
accuracy, and the robust-minus-naive accuracy difference — carries a {{CI_PERCENT}}%
...
{{SWEEP_BEST_MEAN_ACC_DIFF}} with {{CI_PERCENT}}% CI
$[{{SWEEP_BEST_MEAN_ACC_DIFF_CI_LO}}, {{SWEEP_BEST_MEAN_ACC_DIFF_CI_HI}}]$, and
the accuracy at the verdict rate is {{SWEEP_NAIVE_VERDICT_ACCURACY_MEAN}} for the
naive pool ({{CI_PERCENT}}% CI
$[{{SWEEP_NAIVE_VERDICT_ACCURACY_CI_LO}}, {{SWEEP_NAIVE_VERDICT_ACCURACY_CI_HI}}]$)
against {{SWEEP_BEST_VERDICT_ACCURACY_MEAN}} for the most accurate robust member ({{CI_PERCENT}}% CI
...
$\alpha = {{STATISTICS_FDR_ALPHA}}$. The verdict family owns one
...
method has raw
$p = {{SWEEP_BEST_RAW_PVALUE_MATH}}$, deflating to BH
...
at the primary sweep's matched-trial unit, $n_{\rm trial} = {{SWEEP_N_TRIALS}}$,
$\alpha = {{CONFIG_POWER_ALPHA}}$, alternative `{{CONFIG_POWER_ALTERNATIVE}}`, is
{{SWEEP_HEADLINE_POWER}}; the prospective sample size for the target power
{{CONFIG_TARGET_POWER}} at the observed effect is
$n_{\rm trial} = {{SWEEP_HEADLINE_N_FOR_TARGET_POWER}}$. The estimator is the deterministic
...
|---|---|---|
| log-linear pooling | ${{COMPLEXITY_LOG_TIME_ORDER}}$ | ${{COMPLEXITY_LOG_MEMORY_ORDER}}$ |
| iterative robust pooling | ${{COMPLEXITY_ROBUST_TIME_ORDER}}$ | ${{COMPLEXITY_ROBUST_MEMORY_ORDER}}$ |
| objective-backed variational pooling | ${{COMPLEXITY_VARIATIONAL_TIME_ORDER}}$ | ${{COMPLEXITY_VARIATIONAL_MEMORY_ORDER}}$ |
| self-excluding naive sharing | ${{COMPLEXITY_SHARING_TIME_ORDER}}$ | ${{COMPLEXITY_SHARING_MEMORY_ORDER}}$ |
| self-excluding robust sharing | ${{COMPLEXITY_ROBUST_SHARING_TIME_ORDER}}$ | ${{COMPLEXITY_ROBUST_SHARING_MEMORY_ORDER}}$ |
| one-step state inference | ${{COMPLEXITY_INFER_TIME_ORDER}}$ | ${{COMPLEXITY_INFER_MEMORY_ORDER}}$ |
| server round, excluding queue/network wait | ${{COMPLEXITY_SERVER_TIME_ORDER}}$ | ${{COMPLEXITY_SERVER_MEMORY_ORDER}}$ |
...
declared grids $N \in \{{{COMPLEXITY_AGENT_GRID}}\}$, $S \in
\{{{COMPLEXITY_STATE_GRID}}\}$, self-excluding sharing $N \in
\{{{COMPLEXITY_SHARING_AGENT_GRID}}\}$, and $M \in
\{{{COMPLEXITY_MODALITY_GRID}}\}$. The fixed dimensions are
$N={{COMPLEXITY_FIXED_AGENTS}}$, $S={{COMPLEXITY_FIXED_STATES}}$ for aggregation,
and $S={{COMPLEXITY_INFERENCE_STATES}}$ for state inference; direct aggregation
and server timings use $I={{COMPLEXITY_MAX_ITER}}$, while the public robust
self-excluding sharing path uses its solver budget $I={{COMPLEXITY_ROBUST_SHARING_ITERATIONS}}$;
the default variational path uses $B={{COMPLEXITY_VARIATIONAL_STARTS}}$. Each grid point is warmed up
${{COMPLEXITY_WARMUPS}}$ time(s) and measured ${{COMPLEXITY_REPEATS}}$ time(s),
with median time plotted and the observed repeat range shown as a min--max bar.
The fixed input seed is ${{COMPLEXITY_SEED}}$ on the ${{COMPLEXITY_MACHINE}}$
machine using Python ${{COMPLEXITY_PYTHON}}$ and NumPy ${{COMPLEXITY_NUMPY}}$.

The measured log--log slopes are descriptive checks of the expected orders, not
performance guarantees: agent-axis slopes are {{COMPLEXITY_AGENT_SLOPE_LOG}}
(log-linear), {{COMPLEXITY_AGENT_SLOPE_ROBUST}} (iterative robust),
{{COMPLEXITY_AGENT_SLOPE_VARIATIONAL}} (variational), {{COMPLEXITY_AGENT_SLOPE_SHARING}}
(naive self-excluding sharing), and {{COMPLEXITY_AGENT_SLOPE_SHARING_ROBUST}}
(robust self-excluding sharing); state-axis slopes
are {{COMPLEXITY_STATE_SLOPE_LOG}}, {{COMPLEXITY_STATE_SLOPE_ROBUST}}, and
{{COMPLEXITY_STATE_SLOPE_VARIATIONAL}}; the modality-axis inference slope is
{{COMPLEXITY_MODALITY_SLOPE}}. The slope fit is a timing diagnostic on this
...
![Implementation-derived complexity and seeded machine-scaling diagnostic. Source relation: original project computational-complexity diagnostic; estimand: median wall-clock time of the real categorical aggregation, naive and robust self-excluding sharing, and state-inference call paths as the declared dimension changes; uncertainty: min--max span over the repeated timings, not a confidence interval; replication unit: fixed seeded input at each grid point with the declared timing repeats. The x-axis is the varied agent, state, or modality dimension, and the y-axis is median wall-clock time in seconds. The panels show agent scaling for the aggregation rules, naive and iterative-robust $N^2$ leave-one-out sharing, state scaling for the aggregators, and modality scaling for one-step inference. Dotted lines are normalized $\Theta$-order guides from the implementation-derived accounting; they are not fitted claims. The experiment ran on ${{COMPLEXITY_MACHINE}}$ with Python ${{COMPLEXITY_PYTHON}}$, NumPy ${{COMPLEXITY_NUMPY}}$, seed ${{COMPLEXITY_SEED}}$, and ${{COMPLEXITY_REPEATS}}$ measured repeat(s) after ${{COMPLEXITY_WARMUPS}}$ warmup(s).](../output/figures/complexity_scaling.png){#fig:complexity-scaling width=95%}
