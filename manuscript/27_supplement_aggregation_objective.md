...
$F = {{VARIATIONAL_SINGLE_START_F}}$ (the higher observed basin, where the
saboteur keeps its weight), while one of the configured alternative starts
reaches a lower observed basin at $F = {{VARIATIONAL_MULTI_START_F}}$ — a gap of
{{VARIATIONAL_CAPTURE_GAP}} nats. This finite multistart comparison shows that
...
The closest observed weight is $\lambda^{\ast} = {{TEMPERED_LAMBDA_STAR}}$
