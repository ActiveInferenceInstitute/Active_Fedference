...
{{ISC_PASSED}} of {{ISC_TOTAL}} acceptance criteria are verified. The
pure-NumPy/SciPy core carries project test coverage of
{{COVERAGE_PERCENT}}% (gate $\ge 90\%$), with every stochastic step threaded
through a single seeded `np.random.default_rng({{EXPERIMENT_SEED}})`.
