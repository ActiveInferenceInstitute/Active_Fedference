...
$\alpha = {{RECOVERY_OFFSWITCH_ALPHA}}$, $\beta = {{RECOVERY_OFFSWITCH_BETA}}$,
...
{{RECOVERY_RCCE_OFFSWITCH_MAXDIFF_MATH}} respectively, and any failure of
...
{{RECOVERY_AGGREGATE_MAXDIFF}} above pins the latter.
...
{{RECOVERY_POSTERIOR_MAXDIFF}}. Pooling such local posteriors has the stated
...
zero at a tolerance of ${{ISC_EFE_TOLERANCE}}$, and each term's semantics is
...
$\lambda = {{TEMPERED_ENTROPY_WEIGHT_DEFAULT}}$ the objective, both block
...
$\lambda={{TEMPERED_ENTROPY_WEIGHT_DEFAULT}}$, the entropy coefficient is the
...
$\lambda={{TEMPERED_ENTROPY_WEIGHT_DEFAULT}}$ slice identifies the
...
$\lambda\in\{{{TEMPERED_LAMBDA_GRID}}\}$ is a finite sensitivity study, not a
...
$\lambda^{\ast}={{TEMPERED_LAMBDA_STAR}}$, with observed gap
${{TEMPERED_LAMBDA_STAR_DIFF}}$. These tokens are computed from the executed
