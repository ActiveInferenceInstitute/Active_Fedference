...
The residual is pinned to ${{ISC_EFE_TOLERANCE}}$. Independent checks require
...
Let $\lambda > 0$ and let $F_\lambda$ be (\ref{eq:tempered-family}). At fixed $a$,
the $q$-minimizer is (\ref{eq:tempered-updates}). At
$\lambda = {{TEMPERED_ENTROPY_WEIGHT_DEFAULT}}$, $F_\lambda$ and both updates equal
