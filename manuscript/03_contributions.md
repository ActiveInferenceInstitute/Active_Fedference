...
\ref{prop:robust-loss-recovery}; $\le {{RECOVERY_BETA_MAXDIFF_MATH}}$ and
$\le {{RECOVERY_RCCE_MAXDIFF_MATH}}$ maximum residual,
[@eq:standard-bayes]).

The Rényi divergence recovers KL (Lemma \ref{lem:renyi-kl-limit};
$\le {{RECOVERY_RENYI_MAXDIFF_MATH}}$ residual, [@eq:renyi-limit]). The
...
accuracy {{SWEEP_BEST_VERDICT_ACCURACY_MEAN}} against the naive pool's
{{SWEEP_NAIVE_VERDICT_ACCURACY_MEAN}} at the verdict rate.

At that rate, $q = {{SWEEP_BEST_QVALUE_MATH}}$, and the
rank-biserial-derived $d$-equivalent is {{SWEEP_BEST_D_EQUIVALENT}}. The
predeclared selection rule is {{SWEEP_HEADLINE_SELECTION_RULE}}; the method with
the largest paired mean difference is
{{SWEEP_LARGEST_MEAN_DIFFERENCE_METHOD}}. Every headline number is a generated
token.
...
At each value in the {{PARAM_RECOVERY_ACUITY_GRID}} acuity grid, the study
generates {{PARAM_RECOVERY_N_OBSERVATIONS}} synthetic observations in each of
{{PARAM_RECOVERY_N_TRIALS}} trials. It selects acuity by marginal-likelihood
grid search over the declared finite grid.

The observed mean absolute error is {{PARAM_RECOVERY_MEAN_ABS_ERROR}} with
$R^2 = {{PARAM_RECOVERY_R_SQUARED}}$ ([@fig:parameter-recovery]).
...
$\lambda = {{TEMPERED_ENTROPY_WEIGHT_DEFAULT}}$, the temperature is unity
...
{{V4_N_AGENTS}} agents each observe a {{V4_FOV_WIDTH}}-slot disjoint window
shows that belief sharing materially improves over isolated-agent accuracy in
the declared configuration. Isolated agents clear the {{V4_CHANCE_BASELINE}}
...
fixes the configuration; [@sec:results] reports the {{N_STUDIES}} studies,
