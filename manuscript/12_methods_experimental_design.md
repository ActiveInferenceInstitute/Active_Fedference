...
single-run studies use the first configured seed ({{EXPERIMENT_SEED}}), and the
...
|---|---|---|
| 1 — Belief sharing | Communicating vs. incommunicado colony free energy | `n_agents = {{BELIEF_SHARING_N_AGENTS}}`, `acuity = {{BELIEF_SHARING_ACUITY}}` |
| 2 — Language acquisition | Dirichlet-learning KL descent ([@eq:dirichlet-kl]) | `num_steps = {{LANGUAGE_NUM_STEPS}}` |
| 3 — Emergence / BMR | Reduced-vs-full model evidence ([@eq:bmr-deltaf]) | candidate states `n = {{EMERGENCE_N}}` |
| 4 — Robustness sweep | Robust vs. naive consensus under contamination | `n_agents = {{SWEEP_N_AGENTS}}`, `n_contaminated = {{SWEEP_N_CONTAMINATED}}` |
| 5 — Disjoint-FOV moving world | Communication necessity with non-overlapping fields of view | `n_agents = {{V4_N_AGENTS}}`, `fov_width = {{V4_FOV_WIDTH}}` (Supplement, [@sec:results-moving]) |

: Per-study configuration, read from `experiment:` in `config.yaml` and surfaced as manuscript tokens by `src/manuscript_variables.generate_variables`. The sample sizes carried into the statistics — the across-seed belief-sharing sample ($n = {{BELIEF_SHARING_N_SEEDS}}$ seeds), the language trajectory ({{LANGUAGE_N_POINTS}} ordered points summarized over $n = {{LANGUAGE_N_SEEDS}}$ independent seeds), and the paired robustness trials ($n = {{SWEEP_N}}$ per condition) — are reported with their respective results. {#tbl:study_params}

**Study 1 — belief sharing.** A colony of {{BELIEF_SHARING_N_AGENTS}} sentinels
at the deliberately low acuity {{BELIEF_SHARING_ACUITY}} each infer the creature
...
across-seed sample
is $n = {{BELIEF_SHARING_N_SEEDS}}$ seeds, one colony-mean free energy per seed
...
{{LANGUAGE_NUM_STEPS}} steps. We record the KL descent of [@eq:dirichlet-kl] at
each ordered step, giving {{LANGUAGE_N_POINTS}} points per trajectory and $n =
{{LANGUAGE_N_SEEDS}}$ independent seed trajectories for the pointwise interval
in [@fig:language-kl].

**Study 3 — emergence.** A redundant model reduction and a supported one are
scored by the BMR free energy of [@eq:bmr-deltaf] over $n = {{EMERGENCE_N}}$
candidate states ([@fig:emergence-bmr]).

**Study 4 — robustness sweep.** The sweep varies two factors on a colony of
{{SWEEP_N_AGENTS}} sentinels of which {{SWEEP_N_CONTAMINATED}} are saboteurs
(`confident_wrong`, [@sec:methods-corruption]):

- **Contamination rate** over $\{{{SWEEP_RATES}}\}$ — the convex-mix weight of
...
- **Server robustness setting**, named with FedGVI client-loss/
  ...
  $\{{{CONFIG_DIVERGENCES}}\}$. `KLD` is the non-robust Friston / standard-Bayes
  ...
  $\{{{CONFIG_ROBUST_DIVERGENCES}}\}$ each select a fixed `robust_aggregate`
  down-weighting constant (the executed mapping is {{SWEEP_SERVER_OPERATING_POINTS}};
...
The headline verdict pairs {{SWEEP_N_TRIALS}} independent trials at the fixed
contamination rate {{SWEEP_VERDICT_RATE}} — heavy contamination that degrades the
...
places {{V4_N_AGENTS}} agents on a {{V4_FOV_WIDTH}}-slot disjoint-FOV track to
...
concentrating $n = {{SWEEP_N}}$ paired trials at the single rate
{{SWEEP_VERDICT_RATE}} gives the test the resolution to detect the robustness
effect; scattering the same budget across {{CONFIG_N_RATES}} rates would dilute
every contrast. The across-seed studies are powered separately, with $n =
{{BELIEF_SHARING_N_SEEDS}}$ seeds for belief sharing and $n =
{{LANGUAGE_N_SEEDS}}$ independent seed trajectories for language acquisition.
The {{LANGUAGE_N_POINTS}} ordered learning points are repeated measures within
...
$n = {{CROSS_STUDY_N_SEEDS}}$ independent seeds and
$n = {{CROSS_STUDY_N_TRIALS}}$ matched trials per contamination rate for its
...
uses {{REVIEW_GRID_N_SEEDS}} deterministic seed replicates, with
{{REVIEW_GRID_N_TRIALS}} trials nested within each seed and scenario/rate cell,
and retains the registered rates {{REVIEW_GRID_RATES}} across the finite attack
union {{REVIEW_GRID_ATTACKS}}. Its independent unit is the configured seed
...
observed effect. For the headline robust method ({{SWEEP_HEADLINE_METHOD}}) the
observed-effect design power of the paired Wilcoxon at the run's
$n = {{SWEEP_N}}$, computed at $\alpha = {{CONFIG_POWER_ALPHA}}$ against the
directional alternative `{{CONFIG_POWER_ALTERNATIVE}}` (robust accuracy exceeds
naive), is {{SWEEP_HEADLINE_POWER}}. The power approximation uses the deterministic
...
$n = {{SWEEP_HEADLINE_N_FOR_TARGET_POWER}}$ matched trials — the explicit
