...
|---|---|
| 1 — Belief sharing | `n_agents = {{BELIEF_SHARING_N_AGENTS}}`; `acuity = {{BELIEF_SHARING_ACUITY}}` |
| 2 — Language acquisition | `num_steps = {{LANGUAGE_NUM_STEPS}}` |
| 3 — Emergence / BMR | candidate states `n = {{EMERGENCE_N}}` |
| 4 — Robustness sweep | `n_agents = {{SWEEP_N_AGENTS}}`; `n_contaminated = {{SWEEP_N_CONTAMINATED}}` |
| 5 — Disjoint-FOV world | `n_agents = {{V4_N_AGENTS}}`; `fov_width = {{V4_FOV_WIDTH}}` |
...
($n = {{BELIEF_SHARING_N_SEEDS}}$ seeds), the language trajectory
({{LANGUAGE_N_POINTS}} ordered points over $n = {{LANGUAGE_N_SEEDS}}$ independent
seeds), and the paired robustness trials ($n = {{SWEEP_N}}$ per condition).
{#tbl:study_params}
...
The contamination rate spans $\{{{SWEEP_RATES}}\}$ and is the convex-mix
...
vocabulary for cross-reference only: $\{{{CONFIG_DIVERGENCES}}\}$.
...
The remaining labels $\{{{CONFIG_ROBUST_DIVERGENCES}}\}$ each select a fixed
`robust_aggregate` down-weighting constant. The executed mapping is
{{SWEEP_SERVER_OPERATING_POINTS}}, defined in
...
For the headline robust method ({{SWEEP_HEADLINE_METHOD}}) the
