...
**Definition 1** (Generalized-(Gibbs)-Bayes posterior). For a loss $L$, prior $\pi_0$, learning rate $\tau>0$, and divergence
...
**Definition 2** (Cavity / PVI factor update). The cavity removes agent $n$'s factor from the colony posterior in
...
property `generalized_bayes.cavity` and
`generalized_bayes.update_factor` satisfy.
:::
...
**Lemma 3** (KL is the $\alpha\to1$ limit of the Rényi family). For categorical pmfs $q, p$ on a finite support, the $\alpha$-Rényi divergence
...
`divergences.py` implementation switches to the KL closed form inside a small
...
**Proposition 4** ($\beta$-loss and rcce recover NLL). The recentered density-power loss $L_\beta$ of (\ref{eq:beta-loss}) tends to the
...
**Theorem 5** (Categorical message-combination specialization and local recovery). Let $\mathcal S$ be a finite shared support, let $q_n(s)>0$ for every
...
**Corollary 6** (Closed-form Bayes recovery). With the KL divergence and the NLL loss, the generalized posterior of
...
so `generalized_posterior(KLD, NLL)` reproduces standard Bayes.
...
are 0.96, 0.96, and
0.97; the modality-axis inference slope is
...
**Proposition 7** (Expected-free-energy decomposition identity). For the categorical generative model of `expected_free_energy.py`, the cost
...
**Proposition 8** (Tempered aggregation free energy). Let $\lambda > 0$ be an entropy weight and $F_\lambda$ the objective of
...
263 of 265 acceptance criteria are verified. The
pure-NumPy/SciPy core carries project test coverage of
91.02% (gate $\ge 90\%$), with every stochastic step threaded
...
| Reproducible build epoch (UTC) | 2026-08-27T19:26:00Z |
...
- Acceptance criteria: 265 total, 263 passing.
- Project test suite: 2244 collected cases; the bound successful
  receipt records zero failed cases. The project no-mocks policy remains a
  separately executable source contract.
- Line coverage on `src/`: 91.02% (achieved by the bound full
...
**Proposition 9** (Scoped separable raw-log-pool no-go). For categorical state dimension $K\geq2$, no objective of the declared displayed form
...
**Definition 10** (Aggregation free energy). For $c>0$, $\lambda>0$, consensus $q$, and effective weights
...
::: {.theorem-box .theorem #thm:variational-aggregation}
**Theorem 11** (Variational aggregation: descent, recovery, and effective-weight bound). Let $c>0$ and $\lambda>0$. Each alternating update in
