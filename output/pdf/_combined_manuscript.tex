...
(log-linear), 0.94 (iterative robust), 0.71 (variational), 1.60 (naive
self-excluding sharing), and 1.93 (robust self-excluding sharing);
state-axis slopes are 0.41, 0.39, and 0.42; the modality-axis inference
...
carries project test coverage of 90.69\% (gate \(\ge 90\%\)), with every
...
\item
  Project test suite: 1692 collected cases; the bound successful receipt
  records zero failed cases. The project no-mocks policy remains a
  separately executable source contract.
\item
  Line coverage on \texttt{src/}: 90.69\% (achieved by the bound full
