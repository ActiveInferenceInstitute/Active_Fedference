...
\ifXeTeX
\special{pdf:trailerid [ <ce3023567a20df4bfcb17882952ee3cc> <ce3023567a20df4bfcb17882952ee3cc> ]}
\fi
\ifPDFTeX
\pdftrailerid{}
\pdftrailer{/ID [ <ce3023567a20df4bfcb17882952ee3cc> <ce3023567a20df4bfcb17882952ee3cc> ]}
\fi
\ifLuaTeX
\pdfvariable trailerid {[ <ce3023567a20df4bfcb17882952ee3cc> <ce3023567a20df4bfcb17882952ee3cc> ]}
\fi
...
orders, not performance guarantees: agent-axis slopes are 0.97
(log-linear), 0.94 (iterative robust), 0.78 (variational), 1.64 (naive
self-excluding sharing), and 1.91 (robust self-excluding sharing);
state-axis slopes are 0.96, 0.96, and 0.96; the modality-axis inference
slope is 0.65. The slope fit is a timing diagnostic on this machine, not
...
(at \(\alpha = 1.00001\), \(\beta = 1.00 \times 10^{-6}\),
...
claim. The posterior row is a measured identity on the general code
path, so its near-zero residual is itself the falsification surface. The
...
265 of 267 acceptance criteria are verified. The pure-NumPy/SciPy core
carries project test coverage of 91.13\% (gate \(\ge 90\%\)), with every
...
rigorous evidence. When PyTorch is not installed, the pipeline records
unavailable-value sentinels; certified builds install the \texttt{torch}
optional extra (sec.~\ref{sec:reproducibility}).
...
\item
  Project test suite: 2355 collected cases; the bound successful receipt
  records zero failed cases. The project no-mocks policy remains a
  separately executable source contract.
\item
  Line coverage on \texttt{src/}: 91.13\% (achieved by the bound full
