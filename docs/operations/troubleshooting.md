...
`publication.pipeline_freshness`. Schema 4 records the path-free identity of
the exact Template commit required by the source-owned renderer lock, and the
runtime checkout must be clean and match that identity. An older receipt is
replaced only when analysis is recorded, after which hydration and rendering
must be regenerated in order. This binds the renderer revision; it does not
turn a green render into publication authorization or an accessibility
conformance claim.
...
schema 4 omits `generated_at` in unreleased builds. Do not hand-edit the
...
**Cause:** A direct standalone slide render was used without the combined
manuscript aux map, or the canonical post-combined strict refresh failed. The
canonical Template invocation clears a stale aux, builds the combined PDF,
validates its new label map, and refreshes every enabled deck before it
returns. It must not leave publishable `??` markers. Standalone authoring
renders remain fail-open and expose unresolved section references as visible
`sec:*` labels.

**Fix:** Run the complete canonical Template render, not a standalone deck
render. If it fails, keep the derivative unpublished and address the precise
missing-aux or unresolved-label diagnostic; do not rerun merely to consume an
aux file from a previous invocation. See
...
Preflight is intentionally fail-closed. The supplied Template checkout must be
the canonical repository at the exact commit pinned in
`manuscript/config.yaml`, with no staged, unstaged, untracked, or submodule
changes. Use a separate clean worktree; do not reset a shared dirty checkout.
