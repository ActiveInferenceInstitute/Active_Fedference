...
The scripts are the executable process boundary, not a second implementation
of the project. They make local and CI runs composable by standardizing
argument parsing, selected checkout roots, subprocess sequencing, artifact
paths, and exit-status semantics. Core functions remain importable for direct
tests and library integrations, while release commands can enforce receipts
...
That is appropriate at a validation boundary; they still delegate source,
report, and release semantics to `src/`, use canonical expected-artifact
contracts, and fail closed rather than scanning whichever files happen to be
present.
...
| `src/analysis/artifacts.py` | Canonical Stage-02 report, figure, registry, and data-path declarations consumed by output audits; declares paths without scanning or running experiments |
