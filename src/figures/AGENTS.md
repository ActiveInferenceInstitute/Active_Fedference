...
- Inspect every changed figure at native PNG scale and in its final PDF/HTML
  ...
  caption's declared replication unit. A passing generator test is not a
...
`replication_unit`. Figure modules stay pure plotting functions; the meaning of
a rendered artifact lives here. Adding a generator means adding its name to
