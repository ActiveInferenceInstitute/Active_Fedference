"""Source-owned API reference and deterministic glossary generation.
...
entry point used by the thin ``scripts/generate_api_docs.py`` wrapper. The
glossary is built from Python's AST, so standalone generation never imports the
modules being documented or depends on a parent monorepo.
"""
...
class _ApiEntry:
    """One public top-level function or class discovered without importing it."""
...
def _first_paragraph(docstring: str | None) -> str:
    """Return a compact, Markdown-table-safe docstring summary."""
    if not docstring:
        return ""
    paragraph = docstring.strip().split("\n\n", 1)[0]
    summary = " ".join(paragraph.split()).replace("|", r"\|")
    if len(summary) > 200:
...
def _build_api_index(source_root: Path) -> list[_ApiEntry]:
    """Collect public top-level functions and classes from ``source_root``."""
    ...
    for path in paths:
        ...
        try:
            ...
            raise ValueError(f"could not parse API source {path}") from exc
    ...
    entries.sort(key=lambda entry: (entry.module, entry.name, entry.kind))
    return entries


def _build_glossary_markdown(source_root: Path) -> str:
    """Return a deterministic source-derived public API inventory."""
    entries = _build_api_index(source_root)
    preamble = (
        "# Active Fedference source API glossary\n\n"
        "Generated from top-level public functions and classes under `src/` "
        "using AST parsing. Presence in this inventory does not itself make a "
        "symbol stable; consult the API stability policy.\n\n"
    )
    if not entries:
        return f"{preamble}No public APIs detected in `src/`.\n"
    lines = [
        "| Module | Name | Kind | Summary |",
        "| --- | --- | --- | --- |",
    ]
    lines.extend(
        f"| `{entry.module}` | `{entry.name}` | {entry.kind} | {entry.summary} |"
        for entry in entries
    )
    table = "\n".join(lines)
    return f"{preamble}{table}\n"


def _remove_stale_glossary(path: Path) -> None:
    """Remove a stale glossary file after generation fails, without touching dirs."""
    if path.is_file() or path.is_symlink():
        path.unlink(missing_ok=True)


def build_api_reference_markdown() -> str:
    """Return the source-owned static API reference."""
    return API_REFERENCE_TEMPLATE


def run_api_doc_generation(project_root: Path) -> dict[str, str | None]:
    """Generate the source-derived glossary and static API reference.

    ``output/docs/api_glossary.md`` is written only when the standalone AST
    inventory succeeds. A generation failure removes any stale file at that
    exact path and reports ``None``. A directory occupying that name is left
    untouched. ``output/docs/api_reference.md`` is always written when the
    output directory itself is writable.
    """
    output_dir = project_root / "output" / "docs"
    output_dir.mkdir(parents=True, exist_ok=True)

    glossary_target = output_dir / "api_glossary.md"
    glossary_path: Path | None = None
    try:
        glossary_target.write_text(
            _build_glossary_markdown(project_root / "src"),
            encoding="utf-8",
        )
        glossary_path = glossary_target
    except (OSError, ValueError) as exc:
        _logger.warning("API index generation failed: %s", exc)
        _remove_stale_glossary(glossary_target)
